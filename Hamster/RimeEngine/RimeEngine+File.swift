...
extension RimeEngine {
  ...
  static var appGroupUserDataDirectoryURL: URL {
    let appGroupUserDataURL = shareURL.appendingPathComponent(AppConstants.rimeUserPathName, isDirectory: true)
    if !FileManager.default.fileExists(atPath: appGroupUserDataURL.path) {
      do {
        try FileManager.default.createDirectory(at: appGroupUserDataURL, withIntermediateDirectories: true)
      } catch {
