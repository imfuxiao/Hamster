...
class MetadataProvider {
  ...
  init() {
    containerRootURL = FileManager.iCloudDocumentURL
    ...
    querySubscriber = Publishers
      ...
      .sink { notification in
        ...
        for item in items {
          ...
          Logger.shared.log.debug("downloading \(item.url)")
        }
      }
    ...
    metadataQuery.start()
    Logger.shared.log.debug("metadataQuery start")
  }
    
  deinit {
    guard metadataQuery.isStarted else { return }
    metadataQuery.stop()
    Logger.shared.log.debug("metadataQuery stop")
  }
}
