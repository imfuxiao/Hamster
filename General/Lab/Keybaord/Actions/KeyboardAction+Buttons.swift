...
public extension KeyboardAction {
   ...
   */
  func hamsterButtonImage(for context: KeyboardContext, rimeContext: RimeContext) -> Image? {
    switch self {
    case .image(_, _, let imageName):
      switch imageName {
      case KeyboardConstant.ImageName.switchLanguage:
        return Image(rimeContext.asciiMode ?
          KeyboardConstant.ImageName.englishLanguageImageName :
            KeyboardConstant.ImageName.chineseLanguageImageName)
      default:
        return nil
      }
    default: return nil
