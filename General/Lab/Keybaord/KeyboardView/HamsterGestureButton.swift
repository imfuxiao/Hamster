 ...
 */
public struct HamsterGestureButton<Label: View>: View {
  /**
  ...
  @EnvironmentObject
  private var actionHandler: HamsterKeyboardActionHandler
...
private extension HamsterGestureButton {
      ...
      Color.clear
        ...
        .gesture(
          DragGesture(minimumDistance: 0)
            .onChanged { value in
              tryHandlePress(value, in: geo)
              if !actionHandler.isScrolling {
                dragAction?(value)
              }
              // fix 候选栏水平滚动期间误触的按键，停止滚动后显示为按压状态
              // 超过一定时间后重置
              DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                if actionHandler.isScrolling, isPressed {
                  isPressed = false
                }
              }
            }
...
private extension HamsterGestureButton {
  func tryHandlePress(_ value: DragGesture.Value, in geo: GeometryProxy) {
    if isPressed { return }
    isPressed = true
    pressAction?()
    if !actionHandler.isScrolling {
